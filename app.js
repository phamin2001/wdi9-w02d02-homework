...
// 1
let filterArrayByLanguage = tagMap.filter(function (item) {
    return item.Language === "en";
});

console.log(filterArrayByLanguage);

// 2.
tagMap.forEach(function (item) {
    if(item.Language === "fr") {
        console.log(item);
    }
}); 
...
// 3.
function tagIdisNotZero(element, index, array) {
    return element.TagID !== 0;
}

console.log(tagMap.every(tagIdisNotZero));

// 4.

 let  tagId = function(element) {
     return element.TagID === 46;
 }
 console.log(tagMap.some(tagId));

 // 5.
 let arrayOfString = tagMap.map( obj => {
    let string = "";
    string = obj.TagName + " " + obj.Language;
    return string;
 });
